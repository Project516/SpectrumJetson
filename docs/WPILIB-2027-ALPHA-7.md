...
The current robot and Jetson pins are the current bench-tested combination. The robot uses WPILib
2027.0.0-alpha-6 with PhotonLib v2027.0.0-alpha-2. The Jetson runs the 2026
FRC-Team-4143 CUDA fork, based on WPILib 2026.2.1. The robot-network integration test
has not passed yet.
...
warn that alpha-6 and earlier vendordeps do not work with alpha-7. No matching
...
4. Flash SystemCore image 14 and install FIRST Driver Station alpha-7.
5. Run the camera, AprilTag, pose, time-sync, NetworkTables, and memory tests
   on the Jetson.
6. Test the Jetson and SystemCore together on the competition network.
